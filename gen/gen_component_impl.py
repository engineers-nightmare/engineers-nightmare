...
header_template_1_stub="""#pragma once
...
%s_component_stub %s_stub_from_config(const config_setting_t *%s_config);
...
%s_component_stub
%s_component_from_config(const config_setting_t *%s_config) {
    %s_component_stub %s_stub{};
"""
...
void
%s_component_stub::register_generator() {
    component_stub_generators["%s"] = %s_component_from_config;
}
    ...
    for fl in files:
        ...
        with open(fl, 'r') as f:
            ...
            for l in f:
                parts = l.strip().split(',')
                if parts[0] == 'body':
                    body_fields.append({'type': parts[1], 'name': parts[2], 'prev': prev, 'comp_name': component_name})
                    prev = parts[2]
                elif parts[0] == 'stub':
                    stub_fields.append({'otype': parts[1], 'type': parts[2], 'name': parts[3], 'comp_name': component_name})

        with open("src/component/%s_component.h" % component_name, "w") as g:
            g.write(header_template_1_stub % component_name)
            for fi in stub_fields:
                g.write(header_template_2_stub % fi)
            g.write(header_template_1 % (component_name, component_name, component_name, component_name))
        ...
        with open("src/component/%s_component.cc" % component_name, "w") as g:
            g.write(impl_template_1_stub % (component_name, component_name, component_name, component_name, component_name, component_name))
            for fi in stub_fields:
                g.write(impl_template_2_stub % fi)
            g.write(impl_template_1 % (component_name, component_name, component_name, component_name, component_name))
            for fi in body_fields:
